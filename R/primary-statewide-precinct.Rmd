---
title: Parse the primary, statewide elections at precinct level
author: Duncan Garmonsway
output: html_document
---

NOTE: The worksheet `warrick` in the file `04_pri_fed.xlsx` must be dealt with
by a human, because it does not state for which office Nancy Warrick was a
candidate.  This script ignores it altogether.
...
```{r, echo = TRUE}
x <- tidyxl::tidy_xlsx("../xlsx/1996/96pr_pct.xlsx")
cells <- x$data[[1]]
corner <-
  cells %>%
  filter(col == 1,
         str_trim(tolower(character)) == "counties")
top_border <- which(!is.na(x$formats$local$border$top$style))
bold <- which(x$formats$local$font$bold)

table_votes <- function(corner, cells, bold, top_border) {
  county_total <-
    cells %>%
    filter(str_trim(character) == "CO. TOTAL")
  area <-
    cells %>%
    filter(row > corner$row,
           row < max(county_total$row),
           col == 1,
           !is.na(character)) %>%
    anti_join(county_total, by = "row")
  county <-
    area %>%
    filter(local_format_id %in% bold,
           !str_detect(tolower(character), "continued")) %>%
    mutate(county = str_trim(tolower(character))) %>%
    select(row, col, county)
  precinct <-
    area %>%
    anti_join(county, by = "row") %>%
    filter(!str_detect(character, "Continued")) %>%
    mutate(precinct = str_trim(tolower(character))) %>%
    select(row, col, precinct)
  party <-
    cells %>%
  ...
  office <-
    cells %>%
    anti_join(party, by = c("row", "col")) %>%
    filter(row < corner$row,
           row > corner$row - 5,
           col != 1,
           (local_format_id %in% bold) | character == toupper(character),
           !is.na(character)) %>%
  ...
  statistics_col <-
    office %>%
    filter(str_detect(tolower(office), "statistic")) %>%
    pull(col) %>%
    min(Inf)
  votes <-
    cells %>%
    semi_join(precinct, by = "row") %>%
...
sheet_votes <- function(cells, name, bold, top_border) {
  if (name %in% c("warrick", "chart1")) {
    # "warrick" is in 04_pri_fed.xlsx, and must be skipped because it
    # doesn't have enough information for even a human to work out what Nancy
    # Warrick was a candidate for.
    # "chart1", in 98pr_stw.xlsx, is a blank sheet.
...
votes %>%
  write_csv(file.path(output_path,
                      "yyyymmdd__id__primary__precinct__statewide.csv"))
```
